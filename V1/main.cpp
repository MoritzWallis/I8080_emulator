...
public:
    byte *registers;
    char registerNames[14][4] = {"B", "C", "D", "E", "H", "L", "W", "Z", "F", "A", "SP", "", "PC", "PSW"};


    RegisterArray(){
        registers = new byte[14];
        memset(registers, 0, 14);
    }
    ~RegisterArray(){
        delete[] registers;
    }
    ...
    word getRegister(byte reg){
        ...
        switch(reg){
            ...
            case 10:
            case 12:
                return (registers[reg] << 8) | registers[reg + 1];
            default:
    ...
    void setRegister(byte reg, word value){
        ...
        switch(reg){
            ...
            case 10:
            case 12:
                registers[reg] = (value >> 8) & 0xFF;
                registers[reg + 1] = value & 0xFF;
                break;
            default:
...
class I8080{
public:
    enum Register {B, C, D, E, H, L, W, Z, F, A, SP, PC=12, PSW}; // F = "condition bits"
